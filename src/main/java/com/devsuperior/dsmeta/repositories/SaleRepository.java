...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query(value = "SELECT obj FROM Sale obj WHERE obj.date BETWEEN :minDate AND :maxDate AND LOWER(obj.seller.name) LIKE LOWER(CONCAT('%', :name, '%'))",
       countQuery = "SELECT COUNT(obj) FROM Sale obj WHERE obj.date BETWEEN :minDate AND :maxDate AND LOWER(obj.seller.name) LIKE LOWER(CONCAT('%', :name, '%'))")
